...
Moreover, for encryption scheme we require that for all $m$, the distribution $Enc(sk, m)$ (induced by random choice of $sk$), is pseudorandom. In other words,the encryption scheme is one-time secure. Our \SSOT protocol is presented in \figureref{fig:Cons1ssot}. 
		...
		\begin{minipage}{0.95\linewidth}
			\noindent{
				...
				\begin{itemize}\addtolength{\itemsep}{-6pt}
					\item  A \batchOPRF function $F_k$ with output length $\kappa$, where $k$ is random key.
			...
			\begin{enumerate}
				...
				$\forall$ $\L < i < 2^m$, \SS\ chooses $\tilde{sk}_{i} \from\bool^m$ at random such that all $2^m$ values $\tilde{sk}_{i}, i \in [0, 2^m-1]$ are distinct.				
