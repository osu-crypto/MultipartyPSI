					...
					for (u64 bIdx = binStart; bIdx < binEnd;)
					{
						...
						for (u64 stepIdx = 0; stepIdx < curStepSize; ++bIdx, ++stepIdx)
						{
							...
							if (!bin.isEmpty())
							{
								...
								//compute h1(x) and h2(x)
								block hashOut = mBFHasher[0].ecbEncBlock(bins.mXsets[inputIdx]);

								u64& idx1 = *(u64*)&hashOut;
								idx1 %= mBfSize;

								u64& idx2 = *(u64*)&(hashOut >> 64);
								idx2 %= mBfSize;

								for (u64 hashIdx = 2; hashIdx < mBFHasher.size(); ++hashIdx)
								{
									u64 idx = idx1 + hashIdx*idx2;
									idx %= mBfSize;
...
//	void  OPPRFReceiver::recvPlainBased(u64 IdxP, binSet& bins, std::vector<block>& plaintexts, const std::vector<Channel*>& chls)
//	{
//
//		// this is the online phase.
//		gTimer.setTimePoint("online.recv.start");
