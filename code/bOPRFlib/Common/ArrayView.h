...
namespace bOPRF {
	...
	public:
		...
		//T& operator[](int idx) { if (idx >= mSize) throw std::runtime_error("rt error at " LOCATION); return mData[idx]; }
		T& operator[](u64 idx) const
		{
#ifndef NDEBUG
			if (idx >= mSize) throw std::runtime_error("rt error at " LOCATION); 
#endif
