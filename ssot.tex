...
\subsection{Notation}
In all our protocols, we denote $\kappa$ as the computational security parameter while $\lambda$ denotes a statistical security.\todo{...}
parameter
	...
	\framebox{
		\begin{minipage}{0.95\linewidth}
			...
			\begin{enumerate}
				...
				\item the parties performs a \batchOPRF with sender \SS\ on input $n$ values $\{x_i\}$ and receiver \RR\ on input $x^*$
				\begin{enumerate}
					\item \SS\ receives $n$ random keys $k=\{(k^*,k')\}$, 					
					\item \RR\ receives $sk^*=\widetilde F_k(x^*)$ with no information about the keys $k$
					\item $\forall$ $0\leq i < \L$, \SS\ computes $sk_i = \widetilde F_k(x_i)$.
				\end{enumerate}
...
\todo{\textbf{same $y_i$ uses $3+s$ times in one time pad \Enc=>leak????}}
		...
		\begin{minipage}{0.95\linewidth}
			\noindent{
				...
				\begin{itemize}\addtolength{\itemsep}{-6pt}
					...
					\item max. number of items $\beta$ in $\SS$'s bin , where $\beta$ as defined in Table~\ref{tbl:params}
				\end{itemize}
			}
			...
			\begin{enumerate}
				\addtocounter{enumi}{0}
				\item \RR\ hashes his items $X^*=\{x^*_0, \ldots, x^*_{m-1}\}$ into $1.2m$ bins using Cuckoo hashing defined by 3 hash functions $\{H_1, H_2, H_3\}$. After the certain number of times, if the insertion attempts of the item $x^*_i$ to $[1.2m]$ bins  fails,  \RR\ put this item is in the stash bins in an arbitrary order; otherwise $x^*_i$ is in the bin \#$b(x^*_i) \in [1.2m]$.  
				\\
				...
				\item \SS\ hashes his items $X=\{x_0, \ldots, x_{n-1}\}$ into $1.2m$ bins using 3 hash functions $\{H_1, H_2, H_3\}$. For $i \in [1.2m]$, index all $x_j$ is in bin \#$i$ as $r_{i,j}$, and pad this bin \#$i$ to $\beta$ items using distinct dummy values $r_{i,k}$; for $i \in [s]$, pad this bin \#$i$ by the set $X$.
