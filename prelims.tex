...
\begin{figure}[htb]\centering
	\framebox{
		\begin{minipage}{0.95\linewidth}
			...
			\begin{itemize}
				\item Choose random component for seed to the PRF: $k^*, k$ and give these to the sender.
				\item Give $ F( \P,(k^*,k), x^*)$ to the receiver.
			\end{itemize}
...
\begin{figure}[htb]\centering
	\framebox{
		\begin{minipage}{0.95\linewidth}
			...
			\begin{itemize}
				\item Choose random components for seeds to the PRF: $k^*, k_1, \ldots, k_m$ and give these to the sender. \todo{$1.2m+s$ keys!!!}
...
\framebox{\begin{minipage}{0.95\linewidth}
		...
		The functionality is parameterized by the number of parties $n$,  the size of the parties' sets $m$. 
		\begin{itemize}
			\item On input $X_i=\{x^1_i, \ldots, x^m_i\} \subseteq\{0,1\}^*$ from each party $P_i$.
...
\label{fig:psifunc}
\end{figure} 
\todo{our PSI also works in case that each party have the different size of items. However, the protocol leaks these size.}

In \figureref{fig:setinclfunc} we formally describe the set-inclusion functionality.% we achieve. The sender has $
\begin{figure}[htb]\centering
	\framebox{\begin{minipage}{0.95\linewidth}
			
			The functionality is parameterized by the number of parties $n$,  the size of item set for each party $m_i$. 
			\begin{itemize}
				\item On input $x^*$ for party$ P_0$
				\item On input $X_i=\{x^1_i, \ldots, x^{m_i}_i\} \subseteq\{0,1\}^*$ from each party $P_i, 0<i <n$.
				
				\item Give output whether $x^* \in \bigcap\limits_{i=1}^{n-1} X_i$ to $P_0$.
			\end{itemize}
		\end{minipage}
	}
	\caption{Set-inclusion ideal functionality.}
	\label{fig:setinclfunc}
\end{figure} 
